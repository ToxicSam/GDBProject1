...
public class PlayerController : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        p_DefualtColor = cr_Renderer.material.color;
        ...
        for (int i = 0; i < m_Attacks.Length; i ++)
        {
            ...
            {
                Debug.LogError(attack.AttackName + " has a wind up time that is longer than the amount of time that the player is frozen for");
            }
        }
    }
        ...
        for (int i = 0; i < m_Attacks.Length; i++)
        {
            ...
            if (attack.IsReady())
            {
                if (Input.GetButtonDown(attack.Button))
                {
                    ...
                    DescreseHealth(attack.HealthCost);
    ...
    private IEnumerator UseAttack(PlayerAttackInfo attack)
    {
        ...
        StartCoroutine(ChangeColor(p_DefualtColor, 50));
